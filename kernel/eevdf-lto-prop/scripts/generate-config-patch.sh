...
su builder -c "
  ...
  cp /build/linux-cachyos/linux-cachyos-lts/config-*-eevdf-lto-prop .config
  LLVM=1 LLVM_IAS=1 make menuconfig
  diff -u --label .config --label .config /build/linux-cachyos/linux-cachyos-lts/config-*-eevdf-lto-prop .config > config.patch
